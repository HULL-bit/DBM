...
void _showAnnonceForm(BuildContext context, {dynamic item}) {
  final titleCtrl = TextEditingController(text: item?['titre']);
  final contentCtrl = TextEditingController(text: item?['contenu']);
  final catCtrl = TextEditingController(text: item?['categorie'] ?? 'generale');
  final api = ApiService();
  List<dynamic> targetMembers = item?['cible_details'] is List ? List.from(item['cible_details']) : [];

  showDialog(
    context: context,
    builder: (ctx) => StatefulBuilder(
      builder: (context, setState) => AlertDialog(
        title: Text(item == null ? 'Nouvelle Annonce' : 'Modifier Annonce'),
        content: SingleChildScrollView(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              TextField(controller: titleCtrl, decoration: const InputDecoration(labelText: 'Titre')),
              TextField(controller: contentCtrl, decoration: const InputDecoration(labelText: 'Contenu'), maxLines: 3),
              TextField(controller: catCtrl, decoration: const InputDecoration(labelText: 'Catégorie')),
              const SizedBox(height: 16),
              const Text('Cibler des membres (Optionnel)', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              Wrap(
                spacing: 6,
                children: [
                  ...targetMembers.map((m) => Chip(
                    label: Text(m['nom'] ?? m['username'], style: const TextStyle(fontSize: 11)),
                    onDeleted: () => setState(() => targetMembers.remove(m)),
                    deleteIconColor: AppColors.error,
          ...
          ElevatedButton(
            onPressed: () async {
              try {
                ...
                if (item == null) {
                  await api.post(ApiEndpoints.annonces, data);
                } else {
                  await api.patch('${ApiEndpoints.annonces}${item['id']}/', data);
                }
                Navigator.pop(ctx);
              } catch (e) {
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Erreur: $e')));
...
class AnnoncesScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final isManager = context.read<AuthProvider>().user?.isAdmin == true || context.read<AuthProvider>().user?.isJewrinCommunication == true;

    return GenericListScreen(
      title: 'Annonces',
      endpoint: ApiEndpoints.annonces,
      emptyMessage: 'Aucune annonce importante',
      floatingActionButton: isManager ? FloatingActionButton(
        onPressed: () => _showAnnonceForm(context),
        backgroundColor: AppColors.primaryGold,
        child: const Icon(Icons.add, color: AppColors.white),
      ) : null,
      itemBuilder: (item) {
        final date = item['date_ajout'] != null ? item['date_ajout'].split('T')[0] : '';
        return Card(
          margin: const EdgeInsets.only(bottom: 12),
          child: Padding(
            padding: const EdgeInsets.all(16),
            child: Column(
              ...
              children: [
                Row(
                  ...
                  children: [
                    Expanded(child: Text(item['titre'] ?? 'Annonce', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16))),
                    if (isManager)
                      Row(mainAxisSize: MainAxisSize.min, children: [
                        IconButton(icon: const Icon(Icons.edit, size: 18, color: AppColors.primaryGold), onPressed: () => _showAnnonceForm(context, item: item)),
                        IconButton(icon: const Icon(Icons.delete, size: 18, color: AppColors.error), onPressed: () => _confirmDelete(context, ApiEndpoints.annonces, item['id'])),
                      ]),
                    const Icon(Icons.push_pin, color: AppColors.primaryGold, size: 16),
                  ],
                ),
                const SizedBox(height: 8),
                Text(item['contenu'] ?? item['description'] ?? '', style: const TextStyle(fontSize: 14)),
                if (item['categorie'] != null) ...[
                  const SizedBox(height: 8),
                  _badge(item['categorie'], AppColors.primaryGreen),
                ],
                const SizedBox(height: 12),
                Text(date, style: const TextStyle(fontSize: 11, color: AppColors.textGrey, fontStyle: FontStyle.italic)),
              ],
            ),
          ),
        );
