...
import { Add, CheckCircle, RestartAlt, Mic, Stop, Delete, MenuBook, Visibility, PictureAsPdf, Download, Close, ArrowBack } from '@mui/icons-material'
      ...
      {detail ? (
        <>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3, flexWrap: 'wrap' }}>
            <IconButton onClick={() => setDetail(null)} sx={{ bgcolor: `${C.vert}12`, '&:hover': { bgcolor: `${C.vert}22` } }}>
              <ArrowBack sx={{ color: C.vert }} />
            </IconButton>
            <Box sx={{ flex: 1, minWidth: 200 }}>
              <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', flexWrap: 'wrap' }}>
                <Typography variant="h5" sx={{ color: C.vert, fontWeight: 700 }}>{detail.nom_tere}</Typography>
                <Chip label={detail.statut_display} size="small" color={detail.statut === 'termine' ? 'success' : 'warning'} />
              </Box>
              {canManage && <Typography variant="body2" color="text.secondary">{detail.membre_nom}</Typography>}
            </Box>
            {canManage && (
              detail.statut === 'en_cours' ? (
                <Button variant="outlined" startIcon={<CheckCircle />} onClick={() => handleTerminer(detail)} disabled={saving} sx={{ color: 'success.main', borderColor: 'success.main' }}>
                  Marquer terminé
                </Button>
              ) : (
                <Button variant="outlined" startIcon={<RestartAlt />} onClick={() => handleReprendre(detail)} disabled={saving} sx={{ color: C.or, borderColor: C.or }}>
                  Reprendre
                </Button>
              )
            )}
          ...
          <Paper sx={{ p: 2, mb: 3, borderRadius: 2, border: `1px solid ${C.or}30` }}>
          ...
          <Typography variant="h6" sx={{ color: C.vertFonce, fontWeight: 700, mb: 1.5 }}>
            BINDs ({(detail.binds || []).length})
          </Typography>
          ...
          {canManage && (
            <Paper sx={{ p: 2.5, borderRadius: 2, border: `1px solid ${C.vert}30`, bgcolor: `${C.vert}05` }}>
              <Typography variant="subtitle1" sx={{ fontWeight: 700, color: C.vert, mb: 1.5 }}>
                Nouveau BIND {(detail.binds || []).length + 1}
              </Typography>
